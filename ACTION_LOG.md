...
### 2. [🔄 進行中] render 中呼叫 new Date() 造成效能問題
- **發現者**: A
- **檔案**: src/app/dashboard/driver/page.tsx
- **操作**: 移除 render 中的 new Date() 呼叫，改用 currentTime state + useEffect 定時更新
- **已宣告**: currentTime state（第108-109行）
- **尚未完成**: 移除 render 中的 new Date()，加入 useEffect 每秒更新

### 3. [待 commit] ProgressBar.tsx Runtime Error
- **操作**: isLitNext 從檔案底部移至頂部
- **commit**: 待執行

### 4. [待 commit] PAYUNi notify 缺原子性
- **操作**: 3 個 DB 操作包進 prisma.$transaction()
- **commit**: 待執行

### 5. [待 commit] 費率顯示 5%→3%
- **操作**: driver/page.tsx:1005 文字 "5%" → "3%"
- **commit**: 待執行
...
15. [待處理] README.md 完全過期
16. [待處理] CURRENT_WORK.md 落後 HEAD
...
### 已完成
- [x] Commit 規格文件（QR貴賓單 + 客戶資料庫）

### 緊急待 commit（可一起）
- [ ] Commit ProgressBar.tsx
- [ ] Commit PAYUNi notify 原子性
- [ ] Commit 費率顯示修正
- [ ] 移除 render 中的 new Date()

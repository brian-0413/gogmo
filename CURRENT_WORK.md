...
### 最後 commit
```
9f0b0c3 feat(driver): 接單衝突邏輯重構 + 退單扣點功能
```
...
### [完成] 接單衝突邏輯 + 退單扣點（最新）
- **衝突邏輯**：同類型（接送）60 分鐘內直接阻擋；不同類型 60 分鐘內顯示警告視窗後仍可接
- **警告訊息**：「⚠️ 此單時間與您 XX:XX 的 XXX 行程接近，請確認是否來得及」+ 「接單後退單，系統會扣該單金額的 10% 點數，接單前請務必確認接送日期和時間和已有行程是否衝突哦！」
- **退單扣點**（`POST /api/orders/[id]/cancel`）：司機退單扣 10%，訂單還原 PUBLISHED，可被其他司機接單
...
- **接單衝突邏輯**：同類型60分阻擋（直接拒絕）、混合型60分內警告（確認後可接）、退單扣10%
- **派單方不檢查衝突**：不同派單方可有同航班接機單

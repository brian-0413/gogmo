...
### [完成] 智慧排班系統（最新）
- **時間參數**（`src/lib/scheduling.ts`）：
  - 尖峰時段：06:30-09:00 / 16:00-19:00
  - 行車時間表：雙北-桃園（50/75分）/ 松山（30/50分）
  - `detectAirport()` 從地點識別機場
- **推薦邏輯**：
  - 情境一：送機後推薦接機單（落地時間在司機到機場 -20分 ~ +40分）
  - 情境二：接機後推薦送機單（緩衝 75-135 分鐘）
  - 銜接緊密度：完美銜接 / 需等候 / 時間較趕
- **API**：
  - `GET /api/schedule/recommend`：推薦清單 + 時間軸 + 總收入
  - `POST /api/schedule/confirm`：一次接多單（最多 6 單），失敗回滾
- **UI**：
  - 「智慧排班」按鈕 → 面板含觸發行程、預覽時間軸、推薦卡片
  - 卡片可點選加入排班，實時顯示總收入
  - 銜接緊密度視覺化標籤（綠/橘/紅）
  - 「確認排班」一次接多單
